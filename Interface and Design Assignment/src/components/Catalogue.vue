<template>
  <div class="container py-4">
      ...
      <div v-for="card in paginatedCards" :key="card.id" class="col">
        <div class="card h-100 shadow-sm card-hover" @click="goToCard(card)">
          ...
          <div class="card-body">
            ...
            <small class="text-muted d-block">{{ card.type }} — {{ card.race }}</small>
            <small class="text-muted d-block">{{ card.description?.slice(0, 60) }}...</small>
          </div>
          <div class="card-footer d-flex justify-content-between align-items-center">
            <small class="text-muted">ATK: {{ card.atk ?? '—' }}</small>
            <span class="badge bg-primary">{{ card.attribute }}</span>
          </div>
        </div>
      </div>
    </TransitionGroup>
    ...
    <div v-if="filteredCards.length === 0" class="text-center py-5 text-muted">
      ...
      <button class="btn btn-outline-primary" @click="search = ''; filterType = ''">Clear filters</button>
    </div>
...
// All unique types for filter dropdown
...
const filteredCards = computed(() => {
  ...
  if (sortBy.value === 'popularity')  result = [...result].sort((a,b) => (b.popularity||0) - (a.popularity||0))
  if (sortBy.value === 'newest')      result = [...result].sort((a,b) => new Date(b.dateAdded) - new Date(a.dateAdded))
